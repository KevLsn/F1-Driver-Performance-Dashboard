...
class RaceEngine:
    ...
    def compute_lap_time(self, compound: str, stint_lap: int, current_lap: int) -> float:
        ...
        tyre_time = base + degradation * stint_lap
        fuel_time = -self.fuel_coef * current_lap
    ...
    def simulate_strategy(
        ...
        for lap in range(1, self.race_laps + 1):
            ...
            lap_time = self.compute_lap_time(
                compound=compound,
                stint_lap=stint_lap,
                current_lap=lap
            )
    ...
    def simulate_monte_carlo(
        ...
        return {
            "mean_time": float(np.mean(results)),
            "std_time": float(np.std(results))
        }